<script>
   ...
   let tooltipData = {}
   tooltipData = {
      content: `<div class="tooltipContent">${description}</div>`,
      theme: 'just',
...
<div class="tree-view-item" style="--padding-left : {paddingLeft}px" use:tooltipAction={tooltipData}>
   <div class="header-bkg focusable" tabindex="{-1}" role="button"
        on:focus={hello} on:keydown={onKeyDown}  bind:this={headerElement}
      ...
      <div class="header" class:leafHeader="{!isFolder}">
         ...
         {/if}
         <span class:folderLabel="{isFolder}" class:leafLabel="{!isFolder}">{@html label}</span>
      </div>
   </div>
   {#if items && !folded}
      {#each items as item (item.id)}
         <svelte:self items="{item.items}"
                      ...
                      folded="{!item.isOpen}"
                      on:leafSelected
                      on:moveToParent={focus}></svelte:self>
      {/each}
   {/if}
</div>
